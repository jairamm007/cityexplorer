...
const CreateCity = () => {
  ...
  const pageTitle = useMemo(() => (isEditMode ? 'Edit City' : 'Add a New City'), [isEditMode]);
  const pageDescription = useMemo(
    () =>
      isEditMode
        ? 'Update the city details you previously added, then save your changes.'
        : 'Fill in the city details below, then save to publish it in CityExplorer.',
    [isEditMode]
  );
  ...
  useEffect(() => {
    ...
    const loadCity = async () => {
      setLoadingCity(true);
      try {
        const response = await api.get(`/cities/${editId}`);
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      <div className="rounded-[40px] bg-white p-8 shadow-xl">
        ...
        <h1 className="mt-6 text-3xl font-semibold text-slate-900">{pageTitle}</h1>
        ...
        <form onSubmit={handleSubmit} className="grid gap-5 lg:grid-cols-2">
          ...
          <label className="block lg:col-span-2">
            ...
            <p className="mt-2 text-xs text-slate-500">
              Optional. You can either paste Image URL above, upload a photo, or use both. Upload supports up to 20MB.
            </p>
