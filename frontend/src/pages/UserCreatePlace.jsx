...
const CreatePlace = () => {
  ...
  const initialKind = searchParams.get('type') === 'restaurant' ? 'restaurant' : 'attraction';
  ...
  const [placeForm, setPlaceForm] = useState({
    ...initialPlaceForm,
    category: initialKind === 'restaurant' ? 'Restaurant' : 'Attraction',
  });
  ...
  useEffect(() => {
    ...
    const loadPlace = async () => {
      ...
      try {
        const response = await api.get(`/attractions/${editId}`);
        const place = response.data;
        const nextKind = /restaurant/i.test(place.category) ? 'restaurant' : 'attraction';
        setKind(nextKind);
        setSearchParams({ type: nextKind, edit: editId });
        setPlaceForm({
          cityId: place.cityId?._id || place.cityId || '',
          name: place.name || '',
          description: place.description || '',
          location: place.location || '',
          category: place.category || (nextKind === 'restaurant' ? 'Restaurant' : 'Attraction'),
    ...
    loadPlace();
  }, [editId, isEditMode, navigate, setSearchParams]);

  const updateKind = (nextKind) => {
    setKind(nextKind);
    const params = { type: nextKind };
    if (editId) {
      params.edit = editId;
    }
    ...
    setPlaceForm((prev) => ({
      ...prev,
      category: nextKind === 'restaurant' ? 'Restaurant' : 'Attraction',
    }));
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = isEditMode
        ? await api.put(`/attractions/${editId}`, payload, {
      ...
      toast.success(
        `${kind === 'restaurant' ? 'Restaurant' : 'Attraction'} ${
          isEditMode ? 'updated' : 'saved'
        } successfully!`
      );
      navigate(`/attraction/${response.data._id}`);
    } catch (error) {
      const message =
        error.response?.data?.message ||
        error.response?.data?.errors?.[0]?.msg ||
        `Unable to ${isEditMode ? 'update' : 'save'} ${kind}`;
      toast.error(message);
    } finally {
      setSaving(false);
    }
    ...
    <div className="mx-auto max-w-5xl space-y-8">
      <div className="rounded-[40px] bg-white p-8 shadow-xl">
        ...
        <p className="mt-2 text-slate-600">
          {isEditMode
            ? 'Update your attraction or restaurant details, then save your changes to CityExplorer.'
            : 'Choose whether you are adding an attraction or a restaurant, then save it to CityExplorer.'}
        </p>

        <div className="mt-6 flex flex-wrap gap-3">
          <button
            type="button"
            onClick={() => updateKind('attraction')}
            className={`rounded-full px-5 py-3 text-sm font-semibold transition ${
              kind === 'attraction' ? 'bg-slate-900 text-white' : 'border border-slate-200 text-slate-700 hover:border-slate-900 hover:text-slate-900'
            }`}
          >
            Add attraction
          </button>
          <button
            type="button"
            onClick={() => updateKind('restaurant')}
            className={`rounded-full px-5 py-3 text-sm font-semibold transition ${
              kind === 'restaurant' ? 'bg-slate-900 text-white' : 'border border-slate-200 text-slate-700 hover:border-slate-900 hover:text-slate-900'
            }`}
          >
            Add restaurant
          </button>
      ...
      <section className="rounded-[40px] bg-white p-8 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="grid gap-5 lg:grid-cols-2">
          ...
          {[
            { name: 'name', label: kind === 'restaurant' ? 'Restaurant name' : 'Attraction name' },
            ...
            { name: 'ticketPrice', label: kind === 'restaurant' ? 'Average price' : 'Ticket price' },
            { name: 'bestTimeToVisit', label: kind === 'restaurant' ? 'Best time to go' : 'Best time to visit' },
          ].map((field) => (
          ...
          <label className="block lg:col-span-2">
            ...
            <p className="mt-2 text-xs text-slate-500">
              Optional. Provide Image URL, upload photo, or both. Upload supports up to 20MB.
            </p>
          ...
          <label className="block lg:col-span-2">
            ...
            <textarea
              ...
              rows="5"
              className="mt-2 w-full rounded-[28px] border border-slate-200 bg-slate-50 px-4 py-3 outline-none transition focus:border-slate-900"
              placeholder={kind === 'restaurant' ? 'Describe the food, vibe, and why people should visit.' : 'Describe the experience and what makes this place special.'}
            />
          </label>
          ...
          <label className="block lg:col-span-2">
            ...
            <textarea
              ...
              rows="4"
              className="mt-2 w-full rounded-[28px] border border-slate-200 bg-slate-50 px-4 py-3 outline-none transition focus:border-slate-900"
              placeholder={kind === 'restaurant' ? 'Share reservation tips, best dishes, or peak-hour advice.' : 'Share transport tips, timing tips, or what to carry.'}
            />
          </label>
          ...
          <div className="lg:col-span-2 flex flex-wrap gap-3">
            ...
            >
              {saving ? `${isEditMode ? 'Updating' : 'Saving'} ${kind}...` : `${isEditMode ? 'Update' : 'Save'} ${kind}`}
            </button>
            <button
              type="button"
              onClick={() => {
                setPlaceForm({
                  ...initialPlaceForm,
                  category: kind === 'restaurant' ? 'Restaurant' : 'Attraction',
                });
                setImageFile(null);
                setImagePreviewUrl('');
                setCropImageSrc('');
